...
CREATE TABLE IF NOT EXISTS `blog_user`(
   ...
   `user_level` TINYINT(2) NOT NULL COMMENT '用户等级',
   ...
   `user_status` TINYINT(2) NOT NULL COMMENT '用户状态(1激活 0未激活 2小黑屋)',
   PRIMARY KEY ( `user_id` ),
...
CREATE TABLE IF NOT EXISTS `blog_cate`(
   ...
   `cate_img` VARCHAR(255) NOT NULL COMMENT '类目img',
   `cate_order` TINYINT(3) UNSIGNED NOT NULL COMMENT '类目排序',
   `cate_show` TINYINT(2) UNSIGNED NOT NULL COMMENT '是否展示',
   PRIMARY KEY ( `cate_id` ),
...
CREATE TABLE IF NOT EXISTS `blog_article`(
   ...
   `article_img` VARCHAR(255) NOT NULL COMMENT '文章img',
   ...
   `article_browse_count` BIGINT(20) UNSIGNED NOT NULL COMMENT '浏览次数',
   `article_like_count` BIGINT(20) UNSIGNED NOT NULL COMMENT '点赞次数',
   `article_status` TINYINT(2) UNSIGNED NOT NULL COMMENT '状态(1发布0草稿2待审核)',
   `article_comment_status` TINYINT(2) UNSIGNED NOT NULL COMMENT '是否评论1是0否',
   `article_password` CHAR(32) COMMENT '文章密码',
   `article_order` TINYINT(2) UNSIGNED NOT NULL COMMENT '文章排序',
...
CREATE TABLE IF NOT EXISTS `blog_comment`(
   ...
   `comment_status` TINYINT(2) NOT NULL COMMENT '状态0审核1发布2',
...
CREATE TABLE IF NOT EXISTS `blog_tag`(
   ...
   `tag_order` TINYINT(2) UNSIGNED NOT NULL COMMENT 'tag排序',
   `tag_img` VARCHAR(255) NOT NULL COMMENT '标签img',
   `tag_description` VARCHAR(500) NOT NULL COMMENT '标签描述',
   PRIMARY KEY ( `tag_id` ),
