...
def checkJWT(authorization):
    ...
    if Payload['over_time'] and (timenow - Payload['over_time'] > 10*60): # 当前时间-过期时间10分钟
        data['isOver'] = True
    else: # 如果没有超过10分钟，生成一个新的JWT
        Payload['over_time'] = overtime
        data['newJWT'] = base_64(json.dumps(Header, indent=2))+'.'+base_64(json.dumps(Payload, indent=2))+'.'+Signature
